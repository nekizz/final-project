...
var grpcCmd = &cobra.Command{
	Use:   "grpc",
	Short: "used-service service serve grpc command",
	Run:   runGrpcCommand,
}
...
func runGrpcCommand(cmd *cobra.Command, args []string) {
	...
	logrus.WithFields(logrus.Fields{
		...
		"type":    "grpc",
	}).Info("used-service service server started")
	...
	logrus.WithFields(logrus.Fields{
		"service": "used-service-service",
		"type":    "grpc",
	}).Info("used-service service gracefully shutdowns")
...
func initializeServices(orm *gorm.DB, grpcServer *grpc.Server) *grpc.Server {
	usedServService := used_service.NewService(orm)
	pb.RegisterCompanyServiceServer(grpcServer, usedServService)
