...
var restCmd = &cobra.Command{
	Use:   "rest",
	Short: "account service serve rest command",
	Run:   runRestCommand,
}
...
func runRestCommand(cmd *cobra.Command, args []string) {
	...
	logrus.WithFields(logrus.Fields{
		...
		"type":    "rest",
	}).Info("account service server started")
	...
	logrus.WithFields(logrus.Fields{
		"service": "account-service",
		"type":    "rest",
	}).Info("account service gracefully shutdowns")
}

func initializeGatewayService(ctx context.Context, gw *runtime.ServeMux, endpoint string, opts []grpc.DialOption) {
	companyGwErr := pb.RegisterAccountServiceHandlerFromEndpoint(ctx, gw, endpoint, opts)
	if nil != companyGwErr {
