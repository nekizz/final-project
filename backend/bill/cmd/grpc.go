...
import (
	...
	"github.com/nekizz/final-project/backend/bill/bill"
	pb "github.com/nekizz/final-project/backend/go-pbtype/bill"
)

var grpcCmd = &cobra.Command{
	Use:   "grpc",
	Short: "bill service serve grpc command",
	Run:   runGrpcCommand,
}
...
func runGrpcCommand(cmd *cobra.Command, args []string) {
	...
	logrus.WithFields(logrus.Fields{
		"service": "bill",
		"type":    "grpc",
	}).Info("bill service server started")
	...
	logrus.WithFields(logrus.Fields{
		...
		"type":    "grpc",
	}).Info("bill service gracefully shutdowns")
...
func initializeServices(orm *gorm.DB, grpcServer *grpc.Server) *grpc.Server {
	billService := bill.NewService(orm)
	pb.RegisterBillServiceServer(grpcServer, billService)
